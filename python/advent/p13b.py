...
class PMachine(FiniteStateMachine):
    ...
    def get_result(self):
        assert 'success' in self.get_state(), "Should be in state success_complete, got {}".format(self.get_state())
        assert len(self.carts) == 1
        ccart = list(self.carts.values())[0]
    ...
    def s2_read_input(self):
        ...
        inputfile = 'p13test2' if self.is_test else 'p13'
    ...
    def s23_too_many_rounds(self):
        return self.round_count > 1000000

    def s29_fail_complete(self):
        pass 

    def s30_remove_colliders(self):
        prvlen = len(self.carts)
        kllpri = self.get_current_cart().get_priority()

        self.carts = { idx : crt for idx, crt in self.carts.items() if crt.get_priority() != kllpri }
        assert len(self.carts) == prvlen-2

        self.cart_ids = deque([cid for cid in self.cart_ids if cid in self.carts ])


    def s31_show_final_board(self):
        self.print_board_info()

    def s32_success_complete(self):
        pass    
