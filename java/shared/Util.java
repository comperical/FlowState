...
{ 
	...
	/// Get it?? If you're using this, you're in a QUANDARY
	public static <T> T quandary(boolean a, boolean b, T t1, T t2, T t3)
	{
		if(a)
			{ return t1; }
	...
	public static <A>  A getFromListPair(List<A> leftlist, List<A> rghtlist, int pos)
	{
        ...
        public static class Plier<T>
        {
        	...
        	public static <R> Plier<R> build(R start)
        	{
        		return new Plier<R>(start);	
        	}
        	
        	public <S> Plier<S> ply(Function<T, S> func)
        	{
        		return build(func.apply(item));
        	}	
        	...
        	public <S> S get(Function<T, S> func)
        	{
        		return ply(func).get();	
        	}
        }
